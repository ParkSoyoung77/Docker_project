...
# [핵심] YAML 파일 내의 이미지 주소를 로컬 레지스트리 주소로 치환하여 적용
# 원본 파일은 건드리지 않고 메모리상에서만 주소를 바꿔 apply 합니다.
...
sleep 10
sudo kubectl get pods -o wide # 어느 노드에 배치되었는지 확인하기 위해 wide 옵션 추가
